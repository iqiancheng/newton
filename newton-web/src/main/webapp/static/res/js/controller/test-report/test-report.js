...
angular.module('test-report', ['ngRoute', 'ui.bootstrap'])
    ...
    .factory('testReportService',['$q','$http',function($q, $http){
        return({        	
    ...
    .controller('TestReportController', ['$scope', '$http','testReportService', 'commonService', '$location','$rootScope', '$routeParams', '$timeout', '$filter',
        function($scope, $http, testReportService, commonService, $location, $rootScope, $routeParams, $timeout, $filter) {
    	$scope.currentPage = 1;
        $scope.pageSize = 10;
        ...
        $scope.loadData = function() {
        	testReportService.getReport($routeParams.resultId).then(function (res) {
        		if (res.data.code == 0) {
    	...
    	$scope.cancel = function() {
    		$location.path('testResult/'+$scope.testCaseId+"/"+$scope.host+"/"+$scope.interfaceName+"/"+$scope.methodName+"/"+$scope.clientCount+"/"+$scope.messageCount);
    	};
